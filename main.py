...
# TOML Configuration
TOML_SERVER_PORT = config["server"]["port"]
TOML_LISTEN_PORT = config["server"]["listen_port"]
TOML_LOCAL_LISTEN_PORT = config["client"]["listen_port"]
TOML_LOCAL_APP_PORT = config["client"]["app_port"]
...
def run_server():
    ...
    try:
        ...
        logger.error("Certificate files not found. Ensure server.crt, server.key, and client.crt exist.")
        return

    server = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    server.bind(("0.0.0.0", TOML_LISTEN_PORT))
    server.listen(5)
    logger.info(f"Server waiting for TLSv1.3 connections on port {TOML_LISTEN_PORT}")
    ...
    while True:
        ...
        try:
            ...
            local_app_socket.connect(("127.0.0.1", TOML_LOCAL_APP_PORT))
...
def run_client():
    ...
    try:
        ...
        context.verify_mode = ssl.CERT_REQUIRED
        context.load_verify_locations(cafile=config["tls"]["cert"])
    except FileNotFoundError:
        logger.error(
            "Certificate files not found. Ensure client.crt, client.key, and server.crt exist."
        )
        return

    local_server = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    local_server.bind(("127.0.0.1", TOML_LOCAL_LISTEN_PORT))
    local_server.listen(5)
    logger.info(
        f"Client ready. Configure the local app to connect to 127.0.0.1:{TOML_LOCAL_LISTEN_PORT}"
    )

    while True:
        local_socket, addr = local_server.accept()
        try:
            raw_server_socket = socket.create_connection((server_ip, TOML_SERVER_PORT))
            ssl_server_socket = context.wrap_socket(raw_server_socket, server_hostname="DeskflowServer")
            ...
            logger.info(
                f"network encryption protocol: {ssl_server_socket.version()}"
            )

            threading.Thread(
                target=forward, args=(local_socket, ssl_server_socket), daemon=True
            ).start()
            threading.Thread(
                target=forward, args=(ssl_server_socket, local_socket), daemon=True
            ).start()
        except Exception as e:
